...
class LocationScheduler:
    ...
    def lawyer_is_not_in_different_city(self, date, lawyer_id_1, lawyer_id_2, location):
        if lawyer_id_1 != '' and lawyer_id_2 != '':
            lawyers_1_cases_that_day = db.session.query(MeetingSchedule).filter(
                MeetingSchedule.lawyer_id_1 == lawyer_id_1,
                MeetingSchedule.date == date,
                MeetingSchedule.location != location).all()

            lawyers_2_cases_that_day = db.session.query(MeetingSchedule).filter(
        ...
        for case in ordered_cases:
            ...
            for J_date in quarterly_j_days:
                if been_placed_in_calendar:
                    break
                lawyers_available = self.lawyer_is_not_in_different_city(J_date.date, case.lawyer_id_1,
                                                                         case.lawyer_id_2, 'Jerusalem')
                if lawyers_available:
